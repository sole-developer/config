...
interface FromEnvContext {
	fallback: <Fallback extends string | number | undefined>(fallback?: Fallback) => FromEnvContext
...
export function fromEnv(envVar: keyof typeof process.env) {
	...
	const ctx: FromEnvContext = {
		fallback<Type extends string | number | undefined>(fallbackValue: Type) {
		...
		array<Type extends string | number>(_separator: string) {

...
describe('fromEnv', function () {
	...
	describe('integer()', function () {
		...
		it("returns 3000 for 'PORT=3000' with parseBehavior='error'", function () {
			port.mock(3000)
			const value = fromEnv('PORT').required().integer({ parseBehavior: 'error' })

			expect(value).toBe(3000)
		})

		it("errors if integer() is called with parseBehavior='error' and PORT is not an integer", function () {
			port.mock('abc')
			expect(() => fromEnv('PORT').integer({ parseBehavior: 'error' })).toThrow()
		})
